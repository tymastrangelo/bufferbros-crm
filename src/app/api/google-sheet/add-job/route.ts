// src/app/api/google-sheet/add-job/route.ts
import { google } from 'googleapis';
import { NextResponse } from 'next/server';
import { type Job, type Client, type Vehicle, type Service, type Addon } from '@/lib/types';

console.log("API Route file /api/google-sheet/add-job/route.ts was loaded by Next.js");
...
export async function POST(request: Request) {
  try {
    ...
    await sheets.spreadsheets.values.append({
      ...
      range: 'Sheet1!A1', // Assumes your sheet is named 'Sheet1'. Change if needed.
